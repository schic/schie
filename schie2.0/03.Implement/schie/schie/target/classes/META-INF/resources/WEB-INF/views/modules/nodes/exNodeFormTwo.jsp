			...
			<form:form id="inputForm" modelAttribute="exNode" action="${ctx}/nodes/exNode/save" method="post" class="form-horizontal content-background">
				<div class="content">
					...
					<div class="row">
						...
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">ip</label>
							<div class="col-sm-8">
								<form:input placeholder="ip地址" path="ip" htmlEscape="false" maxlength="128" class="form-control required" />
							</div>
						</div>
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">MC用户名</label>
							<div class="col-sm-8">
								<form:input placeholder="用户名" path="username" htmlEscape="false" maxlength="255" class="form-control  required"/>
							</div>
						</div>
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">MC密码</label>
							<div class="col-sm-8">
								<input id="password" name="password" type="password" value="" maxlength="50" class="form-control  required"/>
								<c:if test="${not empty exNode.id}"><span
										class="help-inline">若不修改密码，请留空。</span></c:if>
							</div>
						</div>
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">http端口</label>
							<div class="col-sm-8">
								<form:input placeholder="MC的http端口" path="httpPort" htmlEscape="false" maxlength="10" class="form-control number required" />
							</div>
						</div>
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">https端口</label>
							<div class="col-sm-8">
								<form:input placeholder="MC的https端口" path="httpsPort" htmlEscape="false" maxlength="10" class="form-control number required" />
							</div>
						</div>
						<div class="form-group col-xs-6">
							<label class="control-label col-sm-4 pull-left">命令监听端口</label>
							<div class="col-sm-8">
								<form:input placeholder="MC的命令监听端口" path="cmdPort" htmlEscape="false" maxlength="10" class="form-control number" />
                                <span class="help-inline">若不清楚，请留空。</span>
							</div>
						</div>
						<div class="form-group col-xs-6">
