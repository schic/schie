...
//        Page<ExResAsk> page = exResAskService.findPageCache(new Page<ExResAsk>(request, response), exResAsk);
        ...
        exResAsk.setCompany(officeService.get(id));
    ...
    public String form(ExResAsk exResAsk, RedirectAttributes redirectAttributes,Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        }else if("edit".equals(request.getParameter("action")) && STATUS_SUBMIT.equals(exResAsk.getStatus())){
            addMessage(redirectAttributes, "资源申请为提交状态不允许编辑！！！");
    ...
    public String save(ExResAsk exResAsk, Model model, RedirectAttributes redirectAttributes,
        ...
        if (!beanValidator(model, exResAsk)) {
            return form(exResAsk,redirectAttributes, model, request, response);
        }
        SystemAuthorizingRealm.Principal principal = (SystemAuthorizingRealm.Principal) SecurityUtils.getSubject()
                .getPrincipal();
