...
public class ExScheduleCallable implements Callable<String> {
    ...
    private static String getRequestBody(ExJob exJob, String jobLogId) {
        ...
        if (exJob.getResAskNode() != null && StringUtils.isNotEmpty(exJob.getResAskNode().getIp())
                && StringUtils.isNotEmpty(exJob.getResAskNode().getHttpsPort())) {
            target.put("api", exJob.getResAskNode().getCmdUrl());
        }
    ...
    private static String getRequestUrl(ExJob exJob) {
        return exJob.getResNode().getCmdUrl();
    }
