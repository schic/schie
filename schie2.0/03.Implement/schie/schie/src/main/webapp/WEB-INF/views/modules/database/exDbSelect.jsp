...
<head>
	...
	<script type="text/javascript">
        ...
        function select(){
			...
			var dbName =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("dbName");
			var dbType =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("dbType");
			var dbUrl =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("dbUrl");
			var dbUser =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("dbUser");
			var dbPwd =  $("#contentTable tbody tr td input.i-checks:checkbox:checked").attr("dbPwd");
...
<body class="gray-bg">
	...
	<form:form id="searchForm" modelAttribute="exDb" action="${ctx}/database/exDb/" method="post" class="form-inline">
		...
		<div class="form-group">
			...
			<span>数据库类型：</span>
				<form:select path="dbType"  class="form-control m-b">
			...
			<span>数据库密码：</span>
				<form:input path="dbPwd" htmlEscape="false" maxlength="100"  class=" form-control input-sm"/>
			<span>排序：</span>
			...
			<span>创建时间：</span>
				<input id="beginCdate" name="beginCdate" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${exDb.beginCdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/> - 
				<input id="endCdate" name="endCdate" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${exDb.endCdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			<span>创建人：</span>
			...
			<span>修改时间：</span>
				<input id="beginMdate" name="beginMdate" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${exDb.beginMdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/> - 
				<input id="endMdate" name="endMdate" type="text" maxlength="20" class="laydate-icon form-control layer-date input-sm"
					value="<fmt:formatDate value="${exDb.endMdate}" pattern="yyyy-MM-dd HH:mm:ss"/>"/>
			<span>修改人：</span>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column dbName">数据库名称</th>
				<th  class="sort-column dbType">数据库类型</th>
				<th  class="sort-column dbUrl">数据库url</th>
				<th  class="sort-column dbUser">数据库用户</th>
				<th  class="sort-column dbPwd">数据库密码</th>
		...
		<c:forEach items="${page.list}" var="exDb">
			<tr>
				...
				<input type="checkbox" id="${exDb.id}"
					...
					dbPwd="${exDb.dbPwd}"
					sort="${exDb.sort}"
					remarks="${exDb.remarks}"
				class="i-checks"></td>
				<td><a  href="#" onclick="openDialogView('查看DB管理', '${ctx}/database/exDb/form?id=${exDb.id}','800px', '500px')">
					${exDb.companyId}
				</a></td>
				<td>
					${exDb.dbName}
				...
				<td>
					<!--shiro:hasPermission name="db:exDb:view"-->
						<a href="#" onclick="openDialogView('查看DB管理', '${ctx}/database/exDb/form?id=${exDb.id}','800px', '500px')" class="btn btn-info btn-sm" ><i class="fa fa-search-plus"></i> 查看</a>
					<!--/shiro:hasPermission-->
				</td>
			</tr>
		</c:forEach>
