...
public final class ExScheduleUtils {
     ...
     * 立即执行任务
     */
    public static String run(Scheduler scheduler, ExJob job) {
        try {
            // 参数
            //JobDataMap dataMap = new JobDataMap();
            //dataMap.put(ExScheduleConst.EXJOB_PROPERTIES, job);

            scheduler.triggerJob(getJobKey(job.getId()));
            return "";
        } catch (SchedulerException e) {
     ...
     * 立即执行任务
     */
    public static String run(Scheduler scheduler, String jobId) {
        try {

            CronTrigger trigger = getCronTrigger(scheduler, jobId);
            if (trigger == null) {
                return "执行失败，未启动任务执行计划";
            }
     ...
     * 暂停任务
     */
    public static String pauseJob(Scheduler scheduler, String jobId) {
        try {
     ...
     * 恢复任务
     */
    public static String resumeJob(Scheduler scheduler, String jobId) {
        try {
