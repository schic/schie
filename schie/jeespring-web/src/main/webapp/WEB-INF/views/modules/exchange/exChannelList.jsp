...
<script type="text/javascript">
    ...
    function onload() {
            $.ajax({
                ...
                type: "post",
                dataType: "json",
                data:{'encryption':encryption,'monUrl':monurl},
                success: function(data){
                    ...
                    for (var i = 0; i < j ; i++) {
                        //当前通道
                        var tbBody = "";
                        tbBody += "<tr class=\"cdata"+i+"\" id=\"ptemplate"+i+"\"><td id=\"name"+i+"\" onClick=\"td_click(this.id)\">"+source.list.dashboardStatus[i].name+"</td>"
                            +"<td id=\"Status\">"+source.list.dashboardStatus[i].state+"</td>"
                            +"<td id=\"Received\">"+source.list.dashboardStatus[i].statistics.entry[0].long+"</td>"
                            +"<td id=\"Filtered\">"+source.list.dashboardStatus[i].statistics.entry[1].long+"</td>"
                            +"<td id=\"Queued\">"+source.list.dashboardStatus[i].queued+"</td>"
                            +"<td id=\"Sent\">"+source.list.dashboardStatus[i].statistics.entry[2].long+"</td>"
                            +"<td id=\"Errored\">"+source.list.dashboardStatus[i].statistics.entry[3].long+"</td>"
                            +"</tr>"
                        ;
                        $("#datas").append(tbBody);
                        var lefttbBody = "";
                        lefttbBody += "<tr class=\"cdata"+i+"\" id=\"ptemplate"+i+"\"><td id=\"name"+i+"\" onClick=\"td_click(this.id)\">"+source.list.dashboardStatus[i].name+"</td>"
                            +"<td id=\"Status\">"+source.list.dashboardStatus[i].state+"</td>"
                            +"<td id=\"Received\">"+source.list.dashboardStatus[i].lifetimeStatistics.entry[0].long+"</td>"
                            +"<td id=\"Filtered\">"+source.list.dashboardStatus[i].lifetimeStatistics.entry[1].long+"</td>"
                            +"<td id=\"Queued\">"+source.list.dashboardStatus[i].queued+"</td>"
                            +"<td id=\"Sent\">"+source.list.dashboardStatus[i].lifetimeStatistics.entry[2].long+"</td>"
                            +"<td id=\"Errored\">"+source.list.dashboardStatus[i].lifetimeStatistics.entry[3].long+"</td>"
                            +"</tr>"
                        ;
                        $("#leftdatas").append(lefttbBody);
                        // 通道下 Source，Destination个数
                        var h = datalist.list.dashboardStatus[i].childStatuses.dashboardStatus.length;
                        var result = new Array();
                        result=datalist.list.dashboardStatus[i].childStatuses.dashboardStatus;
                        for (var t = 0; t <h ; t++) {
                            var tbBody = "";
                            // $nbsp;
                            ...
                            var lefttbBody = "";
                            var leftresult=datalist.list.dashboardStatus[i].childStatuses.dashboardStatus[t].lifetimeStatistics;
                            // \xa0空格符
