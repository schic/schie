...
public class SystemInfo {
    ...
    public static Map usage() {
        Map<String, Long> monitorMap = new HashMap<String, Long>();
        try {
            Runtime r = Runtime.getRuntime();
        ...
        try {
            ...
            monitorMap.put("ramUsage",
                    Math.round(Common.div(memory.getTotal() - memory.getAvailable(), memory.getTotal(), 2) * 100));
            ...
            */
            ...
            // cpu使用率
            monitorMap.put("cpuUsage", Math.round(b / load.length));
        } catch (Exception e) {
            e.printStackTrace();
        }

        /*try {
            List<Map> cpu = cpuInfos(new Sigar());
            double b = 0.0;
            for (Map m : cpu) {
