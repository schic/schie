...
public class JeeSpringTask {
    ...
    public void jeeSpringParams(String params) {
        System.out.println(dateFormat.format(new Date()) + " | "
                + "定时任务调度测试 com.jeespring.modules.job.task.JeeSpringTask.jeeSpringParams(" + params + ")");
    }
    ...
    public void SysUserOnlineCount() {
        ...
        if (!pageSysUserOnline.getList().isEmpty()) {
            SendMailUtil.sendCommonMail(sysConfig.getValue(),
                    "发送在线用户数邮件 SysUserOnlineCount " + pageSysUserOnline.getList().size() + "人",
                    "发送在线用户数邮件 SysUserOnlineCount " + pageSysUserOnline.getList().size() + "人");
        }
    }
