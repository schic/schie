package org.schic.modules.server.service;
...
import org.schic.modules.server.dao.SysServerDao;
...
/**
 * 
 * @Description: 服务器监控Service
 * @author Caiwb
 * @date 2019年5月6日 上午10:21:27
 */
//启用dubbo服务器时，要去掉下面注解
...
public class SysServerService
		extends
			AbstractBaseService<SysServerDao, SysServer>
		implements
			ISysServerService {
	...
	public SysServer getCache(String id) {
		//获取缓存数据
		SysServer sysServer = (SysServer) redisUtils
				.get(RedisUtils.getIdKey(SysServerService.class.getName(), id));
		if (sysServer != null) {
			return sysServer;
		}
		//获取数据库数据
		...
		//设置缓存数据
		redisUtils.set(
				RedisUtils.getIdKey(SysServerService.class.getName(), id),
				sysServer);
		return sysServer;
	}
	...
	public List<SysServer> totalCache(SysServer sysServer) {
		...
		String totalKey = RedisUtils.getTotalKey(
				SysServerService.class.getName(), JSON.toJSONString(sysServer));
		@SuppressWarnings("unchecked")
		List<SysServer> sysServerList = (List<SysServer>) redisUtils
				.get(totalKey);
	...
	@Override
	public Page<SysServer> findPageCache(Page<SysServer> page,
			SysServer sysServer) {
		//获取缓存数据
		String findPageKey = RedisUtils.getFindPageKey(
				SysServerService.class.getName(),
				JSON.toJSONString(page) + JSON.toJSONString(sysServer));
		@SuppressWarnings("unchecked")
		Page<SysServer> pageReuslt = (Page<SysServer>) redisUtils
				.get(findPageKey);
		if (pageReuslt != null) {
			return pageReuslt;
		}
	...
	public void save(SysServer sysServer) {
		...
		//设置清除缓存数据
		redisUtils.remove(RedisUtils.getIdKey(SysServerService.class.getName(),
				sysServer.getId()));
		//清除列表和页面缓存数据
		redisUtils.removePattern(RedisUtils
				.getFindListKeyPattern(SysServerService.class.getName()));
		redisUtils.removePattern(RedisUtils
				.getFinPageKeyPattern(SysServerService.class.getName()));
	}
