...
import org.schic.modules.sys.dao.SysUserOnlineDao;
...
/**
 * 
 * @Description: 在线用户记录Service
 * @author Caiwb
 * @date 2019年5月6日 上午11:15:10
 */
@Service
@Transactional(readOnly = true)
public class SysUserOnlineService
		extends
			AbstractBaseService<SysUserOnlineDao, SysUserOnline> {
	 ...
	 * 上次同步数据库的时间戳
	 */
	private static final String LAST_SYNC_DB_TIMESTAMP = SysUserOnlineService.class
			.getName() + "LAST_SYNC_DB_TIMESTAMP";

	public SysUserOnline getCache(String id) {
		//获取缓存数据
		SysUserOnline sysUserOnline = (SysUserOnline) redisUtils.get(
				RedisUtils.getIdKey(SysUserOnlineService.class.getName(), id));
	...
	public List<SysUserOnline> totalCache(SysUserOnline sysUserOnline) {
		//获取缓存数据
		String totalKey = RedisUtils.getTotalKey(
				SysUserOnlineService.class.getName(),
				JSON.toJSONString(sysUserOnline));
		@SuppressWarnings("unchecked")
		List<SysUserOnline> sysUserOnlineList = (List<SysUserOnline>) redisUtils
				.get(totalKey);
	...
	public SysUserOnline findListFirstCache(SysUserOnline sysUserOnline) {
		...
		String findListFirstKey = RedisUtils.getFindListFirstKey(
				SysUserOnlineService.class.getName(),
				JSON.toJSONString(sysUserOnline));
		SysUserOnline sysUserOnlineRedis = (SysUserOnline) redisUtils
				.get(findListFirstKey);
	...
	public Page<SysUserOnline> findPageCache(Page<SysUserOnline> page,
			SysUserOnline sysUserOnline) {
		//获取缓存数据
		String findPageKey = RedisUtils.getFindPageKey(
				SysUserOnlineService.class.getName(),
				JSON.toJSONString(page) + JSON.toJSONString(sysUserOnline));
		@SuppressWarnings("unchecked")
		Page<SysUserOnline> pageReuslt = (Page<SysUserOnline>) redisUtils
				.get(findPageKey);
		if (pageReuslt != null) {
			return pageReuslt;
		}
			...
			if ("true".equals(redisRun)) {
				try {
					if (!redisUtils.exists(
							RedisUtils.SHIRO_REDIS + ":" + item.getId())) {
						item.setStatus(
								OnlineSession.OnlineStatus.off_line.toString());
						super.save(item);
					}
				} catch (Exception e) {
				}
	...
	public void save(SysUserOnline sysUserOnline) {
		...
		redisUtils.remove(RedisUtils.getIdKey(
				SysUserOnlineService.class.getName(), sysUserOnline.getId()));
		//清除列表和页面缓存数据
		redisUtils.removePattern(RedisUtils
				.getFindListKeyPattern(SysUserOnlineService.class.getName()));
		redisUtils.removePattern(RedisUtils
				.getFinPageKeyPattern(SysUserOnlineService.class.getName()));
	}
	...
	public void syncToDb(OnlineSession onlineSession) {
		Date lastSyncTimestamp = (Date) onlineSession
				.getAttribute(LAST_SYNC_DB_TIMESTAMP);
		if (lastSyncTimestamp != null) {
			...
			boolean isGuest = onlineSession.getUserId() == null
					|| StringUtils.isEmpty(onlineSession.getUserId());
		...
		onlineSession.setAttribute(LAST_SYNC_DB_TIMESTAMP,
		...
		try {
			SysUserOnline sysUserOnline = SysUserOnline
					.fromOnlineSession(onlineSession);
			SysUserOnline sysUserOnlineDb = this.get(sysUserOnline.getId());
			if (sysUserOnlineDb == null) {
				sysUserOnline.setIsNewRecord(true);
			} else {
				sysUserOnline.setStartTimestsamp(
						sysUserOnlineDb.getStartTimestsamp());
			}
			if (StringUtils.isEmpty(sysUserOnline.getDeptName())) {
				User user = UserUtils
						.getByLoginName(sysUserOnline.getLoginName());
