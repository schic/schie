package org.schic.modules.baiduface.rest;
...
import org.schic.common.utils.GsonUtils;
import org.schic.common.utils.HttpUtil;
 ...
 * @author Caiwb
 * @date 2019年5月5日 下午4:58:47
 */
@RestController
@RequestMapping(value = "/rest/face")
@Api(value = "face百度人脸识别API接口")
public class FaceRecognitionRestController extends AbstractBaseController {
	...
	public String match(@RequestBody String imgStr) {
		...
		try {
			...
			String result = HttpUtil.post(url, token, "application/json",
					param);
	...
	public Result search(@RequestBody String imgStr) {
		...
		try {
			String str1 = this.groupGetlist(); //调用百度查询用户组的API接口，将查询返回的数据接收。
			JSONObject baiJsonObject = JSONObject.parseObject(str1); //将数据转换为json对象类型的数据。
			String str = baiJsonObject.getJSONObject("result")
					.getString("group_id_list").replace("\"", "")
					.replace("[", "").replace("]", "");
			...
			String token = FaceRecognitionRestController.getAuth();
			String baiDuResult = HttpUtil.post(url, token, "application/json",
					param);

			JSONObject returnResult = JSONObject.parseObject(baiDuResult); //将百度返回的数据转为json对象类型的数据。
			String user = returnResult.getJSONObject("result")
					.getString("user_list").replace("\"", "").replace("[", "")
	...
	private String groupGetlist() {
		...
		try {
			...
			String param = GsonUtils.toJson(map);
			String token = FaceRecognitionRestController.getAuth();
			return HttpUtil.post(url, token, "application/json", param);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
