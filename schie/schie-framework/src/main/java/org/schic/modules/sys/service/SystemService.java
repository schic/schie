...
import org.schic.common.service.ServiceException;
...
public class SystemService extends AbstractService implements InitializingBean {
	...
	public List<User> findUserByOfficeId(String officeId) {
		List<User> list = (List<User>) CacheUtils.get(UserUtils.USER_CACHE,
				UserUtils.USER_CACHE_LIST_BY_OFFICE_ID + officeId);
		if (list == null) {
			User user = new User();
			user.setOffice(new Office(officeId));
			list = userDao.findUserByOfficeId(user);
			CacheUtils.put(UserUtils.USER_CACHE,
					UserUtils.USER_CACHE_LIST_BY_OFFICE_ID + officeId, list);
		}
		return list;
	}
	...
	public void saveUser(User user) {
		if (StringUtils.isBlank(user.getId())) {
			...
			if (oldUser.getOffice().getId() != null) {
				CacheUtils.remove(UserUtils.USER_CACHE,
						UserUtils.USER_CACHE_LIST_BY_OFFICE_ID
