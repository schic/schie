...
import org.schic.common.utils.PropertiesLoader;
...
public class Global {
	...
	private static String getDbType(String rawUrl) {
		return rawUrl == null
				? null
				: (!rawUrl.startsWith("jdbc:derby:") && !rawUrl.startsWith(
						"jdbc:log4jdbc:derby:")
								? (!rawUrl.startsWith("jdbc:mysql:") && !rawUrl
										.startsWith("jdbc:cobar:")
										&& !rawUrl.startsWith(
												"jdbc:log4jdbc:mysql:")
														? (rawUrl.startsWith(
																"jdbc:mariadb:")
	...
	public static void modifyConfig(String key, String value) {
		try {
			...
			String path = Global.class.getResource("/schie.properties")
					.getPath();
	...
	public static Properties getProperties() {
		...
		try {
			Reader reader = Resources
					.getResourceAsReader("/schie.properties");
			prop.load(reader);
		} catch (Exception e) {
			return null;
		}
