...
public class UploadUtils {
	...
	private String saveFile(FileItem item) {
		String error = "true";
		String fileNameTmp = item.getName();
		String fileExt = fileNameTmp.substring(fileNameTmp.lastIndexOf(".") + 1)
				.toLowerCase();
		...
		if (item.getSize() > maxSize) { // 检查文件大小
			...
			String newFileName;
			if ("".equals(fileNameTmp.trim())) {
				SimpleDateFormat df = new SimpleDateFormat("yyyyMMddHHmmss");
				newFileName = df.format(new Date()) + "_"
						+ new Random().nextInt(1000) + "." + fileExt;
			} else {
				newFileName = fileNameTmp + "." + fileExt;
			}
