...
import org.schic.modules.utils.service.EmailService;
...
public abstract class AbstractBaseController {
	...
	public String bindException(Exception ex, HttpServletRequest request) {
		try {
			mailService.sendMailException("Java后台异常",
					"URL:" + request.getRequestURL() + "<br>QueryString:"
							+ request.getQueryString() + "<br>Exception:"
							+ ex.getMessage() + "<br>");
	 ...
	 */
	@ModelAttribute
	protected void APIHandler(HttpServletRequest request,
		...
		try {
			...
			if (request.getRequestURI()
					.indexOf("/rest/oauth/apiTimeLimiFaild") >= 0) {
				return;
			}
	 ...
	 */
	@ModelAttribute
	protected void RestHandler(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			if (!RedisUtils.isShireRedis()) {
				return;
			...
			if (request.getRequestURI()
					.indexOf("/rest/oauth/checkToken") >= 0) {
				return;
			}
			Result result = oauthService.checkToken(
					request.getParameter("token"), request.getRemoteAddr());
			if (result.getResultCoe().toString() == "-1") {
