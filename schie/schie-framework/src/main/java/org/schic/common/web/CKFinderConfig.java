...
/**
 * 
 * @Description: CKFinder配置
 * @author Caiwb
 * @date 2019年5月5日 下午4:26:04
 */
public class CKFinderConfig extends Configuration {
	...
	public void init() throws Exception {
			...
			for (int i = 0; i < nodeList.getLength(); ++i) {
				...
				if ("enabled".equals(childNode.getNodeName())) {
					this.enabled = Boolean
							.parseBoolean(childNode.getTextContent().trim());
				}
				...
				if ("imgQuality".equals(childNode.getNodeName())) {
					value = childNode.getTextContent().trim();
					value = value.replaceAll("//D", "");
					method = clazz.getDeclaredMethod("adjustQuality",
							new Class[]{String.class});
					method.setAccessible(true);
					this.imgQuality = Float
							.parseFloat(method.invoke(this, value).toString());
				}
				...
				if ("secureImageUploads".equals(childNode.getNodeName())) {
					this.secureImageUploads = Boolean
							.parseBoolean(childNode.getTextContent().trim());
				}
				...
				if ("plugins".equals(childNode.getNodeName())) {
					method = clazz.getDeclaredMethod("setPlugins",
				...
				if ("basePathBuilderImpl".equals(childNode.getNodeName())) {
					method = clazz.getDeclaredMethod("setBasePathImpl",
							new Class[]{String.class});
					method.setAccessible(true);
		...
		method = clazz.getDeclaredMethod("setTypes",
