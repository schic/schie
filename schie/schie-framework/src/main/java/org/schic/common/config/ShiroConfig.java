 ...
 * 版权所有
 */
package org.schic.common.config;
...
import org.schic.common.filter.LogoutFilter;
import org.schic.common.filter.OnlineSessionFilter;
import org.schic.common.filter.SyncOnlineSessionFilter;
import org.schic.common.redis.RedisUtils;
import org.schic.common.security.shiro.session.CacheSessionDAO;
import org.schic.common.security.shiro.session.SessionManager;
import org.schic.modules.sys.dao.OnlineSessionDAO;
import org.schic.modules.sys.dao.OnlineSessionFactory;
import org.schic.modules.sys.security.FormAuthenticationFilter;
import org.schic.modules.sys.security.SystemAuthorizingRealm;
...
public class ShiroConfig {
	...
	public String shiroFilterChainDefinitions(Environment environment,
		...
		string.append("/schieStatic/** = anon\n");
	...
	public RedisCacheManager redisCacheManager(String redisHostName,
			String reidsPassword, int reidsPort, int expireTimeShiro) {
	...
	public SessionManager sessionManager(CacheSessionDAO dao) {
		...
		sessionManager
				.setSessionIdCookie(new SimpleCookie("org.schic.session.id"));
		...
		if ("true".equals(redisRun) && "true".equals(shiroRedis)) {
			try {
				...
				defaultWebSecurityManager
						.setCacheManager(redisCacheManager(redisHostName,
								reidsPassword, redisPort, expireTimeShiro));
			} catch (Exception e) {
				logger.error(
						"RedisUtils run:" + RedisUtils.RUN_MESSAGE
								+ e.getMessage(),
