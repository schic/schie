...
public class Page<T> implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2682915965592973634L;
	...
	private static final String C_PAGENO = "pageNo";
	private static final String C_PAGESIZE = "pageSize";
	private static final String LICLASS = "<li class=\"";
	private static final String AHREF = "\"><a href=\"javascript:";
	...
	public Page(HttpServletRequest request, HttpServletResponse response,
		...
		String no = request.getParameter(C_PAGENO);
		if (StringUtils.isNumeric(no)) {
			CookieUtils.setCookie(response, C_PAGENO, no);
			this.setPageNo(Integer.parseInt(no));
		} else if (request.getParameter("repage") != null) {
			no = CookieUtils.getCookie(request, C_PAGENO);
			if (StringUtils.isNumeric(no)) {
				this.setPageNo(Integer.parseInt(no));
			}
		}
		// 设置页面大小参数（传递repage参数，来记住页码大小）
		String size = request.getParameter(C_PAGESIZE);
		if (StringUtils.isNumeric(size)) {
			CookieUtils.setCookie(response, C_PAGESIZE, size);
			this.setPageSize(Integer.parseInt(size));
		} else if (request.getParameter("repage") != null) {
			no = CookieUtils.getCookie(request, C_PAGESIZE);
			if (StringUtils.isNumeric(size)) {
				this.setPageSize(Integer.parseInt(size));
			}
		...
		// 设置排序参数
		String orderByTmp = request.getParameter("orderBy");
		if (StringUtils.isNotBlank(orderByTmp)) {
			this.setOrderBy(orderByTmp);
		}
	}
	...
	public String toStringPage() {
		...
		sb.append("<span class=\"pagination-info\">显示第 " + startIndex + " 到第 "
				+ endIndex + " 条记录，总共 " + count + " 条记录</span>");
		sb.append(
				"<span class=\"page-list\">每页显示 <span class=\"btn-group dropup\">");
		sb.append(
				"<button type=\"button\" class=\"btn btn-sm btn-primary dropdown-toggle\" data-toggle=\"dropdown\" aria-expanded=\"false\">");
		sb.append("<span class=\"page-size\">" + pageSize
				+ "</span> <span class=\"caret\"></span>");
		sb.append("</button>");
		sb.append("<ul class=\"dropdown-menu\" role=\"menu\">");
		sb.append(LICLASS + getSelected(pageSize, 10) + AHREF + funcName + "("
				+ pageNo + ",10,'" + funcParam + "');\">10</a></li>");
		sb.append(LICLASS + getSelected(pageSize, 25) + AHREF + funcName + "("
				+ pageNo + ",25,'" + funcParam + "');\">25</a></li>");
		sb.append(LICLASS + getSelected(pageSize, 50) + AHREF + funcName + "("
				+ pageNo + ",50,'" + funcParam + "');\">50</a></li>");
		sb.append(LICLASS + getSelected(pageSize, 100) + AHREF + funcName + "("
				+ pageNo + ",100,'" + funcParam + "');\">100</a></li>");
		sb.append(LICLASS + getSelected(pageSize, 1000) + AHREF + funcName + "("
				+ pageNo + ",1000,'" + funcParam + "');\">1000</a></li>");
		sb.append(LICLASS + getSelected(pageSize, 100000000) + AHREF + funcName
				+ "(" + pageNo + ",100000000,'" + funcParam
				+ "');\">全部</a></li>");
		sb.append("</ul>");
		...
		if (pageNo == first) {// 如果是首页
			...
			sb.append(
					"<li class=\"paginate_button previous\"><a href=\"javascript:\" onclick=\""
							+ funcName + "(" + first + "," + pageSize + ",'"
							+ funcParam
							+ "');\"><i class=\"fa fa-angle-double-left\"></i></a></li>\n");
			sb.append(
					"<li class=\"paginate_button previous\"><a href=\"javascript:\" onclick=\""
							+ funcName + "(" + prev + "," + pageSize + ",'"
							+ funcParam
		...
		if (pageNo == last) {
			...
			sb.append(
					"<li class=\"paginate_button next\"><a href=\"javascript:\" onclick=\""
							+ funcName + "(" + next + "," + pageSize + ",'"
							+ funcParam + "');\">"
							+ "<i class=\"fa fa-angle-right\"></i></a></li>\n");
			sb.append(
					"<li class=\"paginate_button next\"><a href=\"javascript:\" onclick=\""
							+ funcName + "(" + last + "," + pageSize + ",'"
							+ funcParam + "');\">"
