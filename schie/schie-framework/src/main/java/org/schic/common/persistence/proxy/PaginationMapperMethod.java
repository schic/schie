...
/**
 * 
 * @Description: 执行代理类，扩展Mybatis的方式来让其Mapper接口来支持.
 * @author Caiwb
 * @date 2019年4月29日 下午2:26:00
 */
public class PaginationMapperMethod {
	...
	public Object execute(Object[] args) {
		...
		if (paginationIndex != null) {
			...
			rowBounds = new RowBounds(page.getFirstResult(),
					page.getMaxResults());
	...
	private void setupCommandType() {
		...
		if (type != SqlCommandType.SELECT) {
			throw new BindingException(
					"Unsupport execution method for: " + commandName);
		}
	}
	...
	private void validateStatement() {
		if (!config.hasStatement(commandName)) {
			throw new BindingException(
					"Invalid bound statement (not found): " + commandName);
		}
		if (!config.hasStatement(commandCountName)) {
			throw new BindingException(
					"Invalid bound statement (not found): " + commandCountName);
		}
	}
}
