...
public class ExDbStandardService extends AbstractBaseService<ExDbStandardDao, ExDbStandard> implements IExDbStandardService{
	...
	public ExDbStandard getCache(String id) {
		//获取缓存数据
		ExDbStandard exDbStandard=(ExDbStandard)redisUtils.get(RedisUtils.getIdKey(ExDbStandardService.class.getName(),id));
		if( exDbStandard!=null) return  exDbStandard;
		//获取数据库数据
	...
	public List<ExDbStandard> totalCache(ExDbStandard exDbStandard) {
		...
		if(exDbStandardList!=null) return exDbStandardList;
		//获取数据库数据
	...
	public List<ExDbStandard> findListCache(ExDbStandard exDbStandard) {
		//获取缓存数据
		String findListKey = RedisUtils.getFindListKey(ExDbStandardService.class.getName(),JSON.toJSONString(exDbStandard));
	...
	public ExDbStandard findListFirst(ExDbStandard exDbStandard) {;
		//获取数据库数据
		List<ExDbStandard> exDbStandardList=super.findList(exDbStandard);
		if(exDbStandardList.size()>0) exDbStandard=exDbStandardList.get(0);
		return exDbStandard;
	}

	public ExDbStandard findListFirstCache(ExDbStandard exDbStandard) {
		//获取缓存数据
		String findListFirstKey = RedisUtils.getFindListFirstKey(ExDbStandardService.class.getName(),JSON.toJSONString(exDbStandard));
		ExDbStandard exDbStandardRedis=(ExDbStandard)redisUtils.get(findListFirstKey);
		if(exDbStandardRedis!=null) return exDbStandardRedis;
		//获取数据库数据
		List<ExDbStandard> exDbStandardList=super.findList(exDbStandard);
		if(exDbStandardList.size()>0) exDbStandard=exDbStandardList.get(0);
		else exDbStandard=new ExDbStandard();
		//设置缓存数据
	...
	public Page<ExDbStandard> findPageCache(Page<ExDbStandard> page, ExDbStandard exDbStandard) {
		//获取缓存数据
		String findPageKey =  RedisUtils.getFindPageKey(ExDbStandardService.class.getName(),JSON.toJSONString(page)+JSON.toJSONString(exDbStandard));
