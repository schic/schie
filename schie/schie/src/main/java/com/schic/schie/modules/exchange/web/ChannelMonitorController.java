...
//    @RequiresPermissions("exchange:exChannel:list")
    @RequestMapping("data")
       ...
       request.setAttribute("encryption",exNode.getEncryption());
       request.setAttribute("monurl",exNode.getMonUrl());
    ...
    public Map<String,Object> channel(HttpServletRequest request,HttpServletResponse response,ExNode exNode) throws Exception {
        exNode.setMonUrl(exNode.getMonUrl()+"/api/channels/statuses");
        JSONObject https = Httpsutils.https(exNode.getEncryption(),exNode.getMonUrl());
