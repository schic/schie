...
public class ExResourcesService extends AbstractBaseService<ExResourcesDao, ExResources> implements IExResourcesService {
	...
	public ExResources getCache(String id) {
		//获取缓存数据
		ExResources exResources=(ExResources)redisUtils.get(RedisUtils.getIdKey(ExResourcesService.class.getName(),id));
		if( exResources!=null) return  exResources;
		//获取数据库数据
	...
	public List<ExResources> totalCache(ExResources exResources) {
		...
		if(exResourcesList!=null) return exResourcesList;
		//获取数据库数据
	...
	public List<ExResources> findListCache(ExResources exResources) {
		//获取缓存数据
		String findListKey = RedisUtils.getFindListKey(ExResourcesService.class.getName(), JSON.toJSONString(exResources));
	...
	public ExResources findListFirst(ExResources exResources) {;
		//获取数据库数据
		List<ExResources> exResourcesList=super.findList(exResources);
		if(exResourcesList.size()>0) exResources=exResourcesList.get(0);
		return exResources;
	}

	public ExResources findListFirstCache(ExResources exResources) {
		//获取缓存数据
		String findListFirstKey = RedisUtils.getFindListFirstKey(ExResourcesService.class.getName(), JSON.toJSONString(exResources));
		ExResources exResourcesRedis=(ExResources)redisUtils.get(findListFirstKey);
		if(exResourcesRedis!=null) return exResourcesRedis;
		//获取数据库数据
		List<ExResources> exResourcesList=super.findList(exResources);
		if(exResourcesList.size()>0) exResources=exResourcesList.get(0);
		else exResources=new ExResources();
		//设置缓存数据
	...
	public Page<ExResources> findPageCache(Page<ExResources> page, ExResources exResources) {
		//获取缓存数据
		String findPageKey =  RedisUtils.getFindPageKey(ExResourcesService.class.getName(), JSON.toJSONString(page)+ JSON.toJSONString(exResources));
